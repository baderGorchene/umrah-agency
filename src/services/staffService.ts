...
export const getStaff = async (trips: Trip[] = []): Promise<Staff[]> => {
  ...
  try {
    ...
    if (error || !data) {
      console.warn('Could not fetch staff from Supabase, returning mock:', error);
      return initialStaff;
    }
    ...
    return data.map((s) => {
      ...
      const avatarUrl = !s.avatar_url || isUnsplash ? DEFAULT_AVATAR_URL : s.avatar_url;
...
export const createStaff = async (staffData: Omit<Staff, 'id'>, trips: Trip[] = []): Promise<Staff | null> => {
  if (!isSupabaseConfigured()) {
    return { ...staffData, id: `staff-${Date.now()}` };
  }
