...
export const getTrips = async (): Promise<Trip[]> => {
  ...
  try {
    ...
    if (error || !tripsData) {
      console.warn('Could not fetch trips from Supabase, returning mock:', error);
      return initialTrips;
    }

    // Fetch counts for pilgrims and guides per trip dynamically
    const { data: pilgrimsData } = await supabase.from('pilgrims').select('trip_id');
    ...
    pilgrimsData?.forEach((p) => {
      if (p.trip_id) pilgrimCounts[p.trip_id] = (pilgrimCounts[p.trip_id] || 0) + 1;
    });
